...
for i, g in enumerate(driver.find_elements_by_class_name("column-day")):
     ...
     for j in driver.find_elements_by_xpath(path_week_day):
          if(j.text == Constants.WEEKDAY):
               ...
               input_going_to_work.send_keys("9:00")
               #退勤時間を入力
               ...
               input_leaving_work.send_keys("18:30")
               #休憩入りの枠追加
               ...
               #休憩入り時間入力（前半）
               path_before_rest_in = "/html/body/div[1]/div[2]/div/div/div/div[2]/div/form/table/tbody/tr[{}]/td[6]/div[1]/input".format(i)
               input_before_rest_in = driver.find_element_by_xpath(path_before_rest_in)
               input_before_rest_in.send_keys("12:00")
               #休憩戻り時間入力（前半）
               path_before_rest_return = "/html/body/div[1]/div[2]/div/div/div/div[2]/div/form/table/tbody/tr[{}]/td[7]/div[1]/input".format(i)
               input_before_rest_return = driver.find_element_by_xpath(path_before_rest_return)
               input_before_rest_return.send_keys("12:45")
               #休憩入り時間入力（後半）
               path_after_rest_in = "/html/body/div[1]/div[2]/div/div/div/div[2]/div/form/table/tbody/tr[{}]/td[6]/div[2]/input".format(i)
               input_after_rest_in = driver.find_element_by_xpath(path_after_rest_in)
               input_after_rest_in.send_keys("17:30")
               #休憩戻り時間入力（後半）
               path_after_rest_return = "/html/body/div[1]/div[2]/div/div/div/div[2]/div/form/table/tbody/tr[{}]/td[7]/div[2]/input".format(i)
               input_after_rest_return = driver.find_element_by_xpath(path_after_rest_return)
               input_after_rest_return.send_keys("18:00")
