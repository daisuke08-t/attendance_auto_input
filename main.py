...
def main():
     try:
          ...
          workbook = xw.Book(Constants.ATTENDANCE_PYTHON_RUN)
          ...
          #      i.submit()
          print("message: 入力処理が完了しました。ブラウザは60秒後にシャットダウンされます。")
          time.sleep(60)
          driver.quit()
          ...
          print('message: Excelの入力内容に誤りがありますので、見直して再実行してください')
     except FileNotFoundError as e:
          print("-----エラーが発生しました------")
          print('type: ' + str(type(e)))
          print('args: ' + str(e.args))
          print('message: Excelファイルが適切なディレクトリに配置されていない可能性があります。')
     except Exception as e:
          print("-----エラーが発生しました------")
          if("<class 'selenium.common.exceptions.ElementClickInterceptedException'>" == str(type(e))):
               print('type: ' + str(type(e)))
               print('args: ' + str(e.args))
               print('message: 予期しないElement（使い方ツアー等）が入ってきました。再実行をしてください')
          elif("<class 'selenium.common.exceptions.WebDriverException'>" == str(type(e))):
               print('type: ' + str(type(e)))
               print('args: ' + str(e.args))
               print('message: Chromedriverが適切なディレクトリに配置されていない可能性があります。')
          else:
               print('type: ' + str(type(e)))
               print('args: ' + str(e.args))
               print('message: 予期しないエラーが発生しました。')
     finally:
          time.sleep(20)
